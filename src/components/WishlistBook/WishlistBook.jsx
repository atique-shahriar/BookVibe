...
import { localStorageDataStore } from '../../utility/localStorage';

const WishlistBook = ({ wishBook }) => {

    const { image, bookName, author, category, rating, tags, totalPages, publisher, yearOfPublishing } = wishBook;
    const handleBookRead = () => {
        localStorageDataStore(wishBook);
        ...
        <div className='flex flex-col md:flex-row border rounded-xl p-6 gap-4'>
            ...
            <div className='space-y-2'>
                ...
                <div className='flex justify-between md:gap-4 text-xs md:text-base'>
                    ...
                    <span className='bg-green-600 px-2 py-2 md:py-0 md:px-4 rounded-xl text-white font-semibold'> <button onClick={handleBookRead}>Read</button> </span>
                </div>
            </div>
        </div>
